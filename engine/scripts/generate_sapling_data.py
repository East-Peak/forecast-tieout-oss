#!/usr/bin/env python3
"""
Generate deterministic example CSV data for the Sapling Industries demo org.

Sapling Industries profile narrative:
...
Produces five CSV files in engine/data/sapling-industries/.

Usage:
    python -m engine.scripts.generate_sapling_data                  # Generate to engine/data/sapling-industries/
    python -m engine.scripts.generate_sapling_data --verify         # Regenerate and diff against existing
    python -m engine.scripts.generate_sapling_data --output-dir /tmp  # Custom output directory
"""
...
SEED = 200  # Different from Sprout/Acme — produces different deal mix
...
# Sapling is $100M scale — Enterprise + Mid-Market mix, no Commercial.
...
# Deal types — Sapling has meaningful renewal book at $100M scale
...
# Fiscal year: Sapling uses Feb-start fiscal (FY26 = Feb 1, 2026 - Jan 31, 2027)
# Q1: Feb 1 - Apr 30, 2026  (in progress as of TODAY=2026-04-06)
...
# Historical quarters (FY25 Q3 and Q4)
...
def generate_companies(n: int = 150) -> list[dict]:
    ...
    for i in range(n):
        ...
        # Sapling: Enterprise + Mid-Market. ~35% Enterprise, ~65% Mid-Market.
        segment_roll = random.random()
        if segment_roll < 0.35:
            segment = "Enterprise"
            emp_count = random.randint(2000, 30000)
        else:
            segment = "Mid-Market"
...
def _get_ae_ids_by_segment(roster: list[dict]) -> dict[str, list[str]]:
    ...
    # Sapling: Enterprise + Mid-Market.
    result: dict[str, list[str]] = {"Enterprise": [], "Mid-Market": []}
    for m in roster:
...
def generate_deals(
    ...
    companies_by_segment: dict[str, list[dict]] = {"Enterprise": [], "Mid-Market": []}
    for c in companies:
    ...
    # Sapling segment helper: ~35% Enterprise, ~65% Mid-Market.
    def _pick_segment() -> str:
        return "Enterprise" if random.random() < 0.35 else "Mid-Market"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Generate deterministic example CSV data for the Sapling Industries demo org.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
