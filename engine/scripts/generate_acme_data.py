#!/usr/bin/env python3
"""
Generate deterministic example CSV data for the Acme SaaS demo org.

Produces five CSV files in engine/data/acme-saas/ that exercise every
feature of the forecast-tieout engine, including intentional dirty data
for the Audit page.

Usage:
    python -m engine.scripts.generate_acme_data                    # Generate to engine/data/acme-saas/
    python -m engine.scripts.generate_acme_data --verify           # Regenerate and diff against existing
    python -m engine.scripts.generate_acme_data --output-dir /tmp  # Custom output directory
"""
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "engine" / "data" / "acme-saas"
ROSTER_PATH = REPO_ROOT / "engine" / "config" / "profiles" / "acme-saas" / "roster.yaml"
...
SEGMENT_DEAL_PARAMS: dict[str, dict[str, float]] = {
    "Enterprise": {"min": 200_000, "max": 800_000, "center": 400_000, "std": 120_000},
    "Mid-Market": {"min": 75_000, "max": 250_000, "center": 150_000, "std": 40_000},
    "Commercial": {"min": 15_000, "max": 75_000, "center": 40_000, "std": 12_000},
}

# Deal types
DEAL_TYPES = ["New Business", "Expansion", "Renewal"]
DEAL_TYPE_WEIGHTS = [0.55, 0.30, 0.15]
...
# Today anchor: 2026-04-06 
TODAY = date(2026, 4, 6)

# Fiscal year: FY26 = Feb 2026 - Jan 2027
# Q1: Feb 1 - Apr 30, 2026  (current quarter)
# Q2: May 1 - Jul 31, 2026
# Q3: Aug 1 - Oct 31, 2026
# Q4: Nov 1 - Jan 31, 2027
FY_Q1_START = date(2026, 2, 1)
...
# Historical quarters (FY26 Q3 and Q4)
...
DEPARTED_AE_IDS = ["AE-X01", "AE-X02", "AE-X03"]
...
def generate_companies(n: int = 150) -> list[dict]:
    ...
    for i in range(n):
        ...
        # Assign segment based on employee count
        segment_roll = random.random()
        if segment_roll < 0.25:
            segment = "Enterprise"
            emp_count = random.randint(2000, 50000)
        elif segment_roll < 0.60:
            segment = "Mid-Market"
...
def _get_ae_ids_by_segment(roster: list[dict]) -> dict[str, list[str]]:
    ...
    result: dict[str, list[str]] = {"Enterprise": [], "Mid-Market": [], "Commercial": []}
    for m in roster:
...
def generate_deals(
    ...
    companies_by_segment: dict[str, list[dict]] = {"Enterprise": [], "Mid-Market": [], "Commercial": []}
    for c in companies:
    ...
    # -----------------------------------------------------------------------
    # 1) Open deals (~200) distributed across stages
    # -----------------------------------------------------------------------
    open_stage_distribution = {
        ...
        "Business Case": 35,
        "Negotiation": 25,
        "Closed Won": 15,  # Recent wins this quarter
    }
    ...
    for stage, count in open_stage_distribution.items():
        ...
        for _ in range(count):
            # Segment distribution: 25% Enterprise, 40% Mid-Market, 35% Commercial
            ...
            if is_won:
                # Recent wins: closed this quarter
    ...
    # -----------------------------------------------------------------------
    # 2) Historical Closed Won (~80) over past 2 quarters
    # -----------------------------------------------------------------------
    for _ in range(80):
        seg_roll = random.random()
        if seg_roll < 0.25:
    ...
    # -----------------------------------------------------------------------
    # 3) Historical Closed Lost (~120) over past 2 quarters
    # -----------------------------------------------------------------------
    for _ in range(120):
        seg_roll = random.random()
        if seg_roll < 0.25:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Generate deterministic example CSV data for the Acme SaaS demo org.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    if args.verify:
        ...
        print(f"Generating Acme SaaS data to {args.output_dir} ...")
